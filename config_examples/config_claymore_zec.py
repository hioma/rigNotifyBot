...
# Комментарии из файла "config_example.py" оставлены к тем значениям, которые вам, скорее всего, нужно будет поменять
...
miners_settings = {
    'miner: zec': { # Имя можно поменять на удобное
        'ip': '127.0.0.1',
        'port': 3333,
        'di': '23', # важный параметр! Определяет, какие карты используется майнером. Если он не задан в конфиге майнера, то пишите 01..N, где N - это количество ваших карт минус 1 (пример: '01234' для пяти карт, '01' - для двух). Если задан -- тогда сами знаете, что писать (как в майнере*)
